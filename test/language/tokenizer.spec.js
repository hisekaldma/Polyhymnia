describe('Tokenizer', function() {
  ...
  it('can tokenize notes', function() {
    var tokens = Polyhymnia.tokenize('C D E# F G Ab B');
  ...
  it('can tokenize chords', function() {
    var tokens = Polyhymnia.tokenize('CM Dm E#dom Fbaug7 Gdim7');
    
    expect(tokens[0].value).toBe('CM');
    expect(tokens[0].type).toBe(tokenType.CHORD);
    expect(tokens[1].value).toBe('Dm');
    expect(tokens[1].type).toBe(tokenType.CHORD);
    expect(tokens[2].value).toBe('E#dom');
    expect(tokens[2].type).toBe(tokenType.CHORD);
    expect(tokens[3].value).toBe('Fbaug7');
    expect(tokens[3].type).toBe(tokenType.CHORD);
    expect(tokens[4].value).toBe('Gdim7');
    expect(tokens[4].type).toBe(tokenType.CHORD);
  });

  it('can tokenize drums', function() {
    var tokens = Polyhymnia.tokenize('x X');
    
    expect(tokens[0].value).toBe('x');
    expect(tokens[0].type).toBe(tokenType.DRUM_TRIGGER);
  ...
  it('can tokenize numbers', function() {
    ...
    expect(tokens[1].value).toBe('0.2');
    expect(tokens[1].type).toBe(tokenType.NUMBER);
    expect(tokens[2].value).toBe('3.14');
    expect(tokens[2].type).toBe(tokenType.NUMBER);
    expect(tokens[3].value).toBe('100.001');
  ...
  it('params can be one letter if they are inside parens', function() {
