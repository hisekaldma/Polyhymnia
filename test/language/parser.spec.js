describe('Parser', function() {
  ...
  it('can parse a simple sequence rule', function() {
    ...
    expect(rules[0].type).toBe(ruleType.SEQUENCE);
    expect(rules[0].definitions[0].sequence[0]).toBe('R2');
    expect(rules[0].definitions[0].sequence[1]).toBe('R1');
  });
  ...
  it('can parse a simple pattern rule', function() {
    ...
    expect(rules[0].name).toBe('R1');
    expect(rules[0].type).toBe(ruleType.PATTERN);
    expect(rules[0].definitions[0].type).toBe(ruleType.DEFINITION);
  ...
  it('doesn\'t parse sequence rules with instruments and notes', function() {
    expect(function() {
      Polyhymnia.parse(Polyhymnia.tokenize('R1 -> Piano: C C C C'));
    }).toThrowError('Expected a rule name');
  });

  it('doesn\'t parse pattern rules with rule names', function() {
    expect(function() {
      Polyhymnia.parse(Polyhymnia.tokenize('R1 => R1 R2'));
    }).toThrowError('Expected an instrument name');
  });
  ...
  it('doesn\'t parse sequences with missing rules', function() {
    expect(function() {
      Polyhymnia.parse(Polyhymnia.tokenize('R1 -> R2 R1'));
  ...
  it('doesn\'t parse empty sequences', function() {
    expect(function() {
      Polyhymnia.parse(Polyhymnia.tokenize('R1 ->'));
    }).toThrowError('Expected a sequence');
  });

  it('doesn\'t parse empty patterns', function() {
    expect(function() {
      Polyhymnia.parse(Polyhymnia.tokenize('R1 =>'));
    }).toThrowError('Expected a pattern');
  });

  it('doesn\'t parse patterns with condition but without instrument', function() {
    expect(function() {
